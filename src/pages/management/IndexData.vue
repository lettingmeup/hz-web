        ...
        <Row type="flex" style="padding-top: 20px">
          ...
          <Col >
            ...
            <Upload v-show="upshow"
              :on-success="getBack1"
              action="http://localhost:8089/index/sideImg.do">
        ...
        <Row>
          <Col>
            ...
            <Upload
              v-show="upshow"
              :on-success="getBack2"
              action="http://localhost:8089/index/newsBacImg.do">
              <Button icon="ios-cloud-upload-outline">更换图片</Button>
            </Upload>
          </Col>
        </Row>
        ...
        <Row type="flex" style="padding-top: 20px">
          ...
          <Col>
            ...
            <Upload
              v-show="upshow"
              :on-success="getBack4"
              action="http://localhost:8089/index/p6Img.do">
              <Button icon="ios-cloud-upload-outline">更换图片</Button>
            </Upload>
          </Col>
        </Row>
...
<script>
  export default {
    ...
    methods: {
      getImg() {
        var vm = this;
        var url = "http://localhost:8089/index/getIndexData.do"
      ...
      conmit(){
        var vm = this;
        var url = "http://localhost:8089/index/updateIndexData.do"
      ...
      getBack1(res, file){
        ...
        if(res.backCode=="0000"){
          ...
          this.all.p2AboutSideImg = "http://49.232.53.207/imgs/"+file.name;
        }else{
      ...
      getBack2(res, file){
        ...
        if(res.backCode=="0000"){
          vm.$Message.success(res.backDesc);
          this.all.p3NewsBackImg = "http://49.232.53.207/imgs/"+file.name;
        }else{
          vm.$Message.error(res.backDesc);
        }
      },
      getBack3(res, file){
        ...
        if(res.backCode=="0000"){
          vm.$Message.success(res.backDesc);
          this.all.p4TeamImg = "http://49.232.53.207/imgs/"+file.name;
        }else{
          vm.$Message.error(res.backDesc);
        }
      },
      getBack4(res, file){
        ...
        if(res.backCode=="0000"){
          vm.$Message.success(res.backDesc);
          this.all.p6Img = "http://49.232.53.207/imgs/"+file.name;
        }else{
          vm.$Message.error(res.backDesc);
        }
      }
